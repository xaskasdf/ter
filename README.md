...
- [x] F5.4g — Multi-token generation with brandon recipe (temp 0.7 + rep_penalty 1.2 + no_repeat_ngram=3). Generated 4 tokens from "hello" → ' capacity +less=' (semantically garbage at 10M params + 9-trit, structurally correct end-to-end). 108s wall for 9 forwards. **First text generated by brandon-tiny on the ternary substrate.**
- [ ] F5.4h — Calibrate output quality vs guide §8 expected. Likely needs per-call scale calibration (current padded-N bias + LUT discretization compounding hurts fidelity).
- [ ] F5.4i — TinyStories Q4_K_M (test the unpacker for tinier validation runs).
