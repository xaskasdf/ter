...
\title{\texttt{ter}: Running Modern {LLMs} on a Balanced-Ternary {CPU}
       ...
       Cores by $1.9\times$ Without Using Them}
...
byte yields 16 useful {MAC}s. We separate architectural claims (proven
in this work) from engineering claims (production end-to-end inference
parity, requires further kernel fusion and persistent attention; per-op
energy in custom silicon, requires {FPGA} prototype to measure).
\end{abstract}
...
The headline finding is positive but with care. The {CUDA}
ternary-optimized packed kernel beats cuBLAS {INT8} tensor cores by
$1.90\times$ on the Llama 1B matmul fabric, despite using no tensor
cores at all. End-to-end inference is a separate engineering problem
(currently $9\times$ slower than \texttt{llama.cpp} \citep{llamacpp},
attributable to kernel-launch overhead and missing fusion), but the
matmul-fabric architectural claim stands. Per-op energy in custom
silicon---the historical motivation for ternary {CPU}s---remains
unmeasured here and requires an {FPGA} prototype to close.
...
\section{End-to-end inference: why it's slower than \texttt{llama.cpp}}

A separate experiment ran the full Llama 1B forward pipeline
({RMSNorm}, {RoPE}, attention, {FFN}, lm\_head, sampling) in {CUDA}
with {INT8} tensor core matmuls. Result: $14.7$ tokens/s ($68$ ms/token).

Reference: \texttt{llama.cpp} Q8\_0 fp16-tensor-core production
inference on the same {RTX 3090}: $130$ tokens/s ($7.7$ ms/token).

The naive end-to-end ternary engine is $9\times$ slower than production
\texttt{llama.cpp}. The gap is \textbf{engineering, not architectural}:

\begin{itemize}
  \item 200+ device-to-host sync points per token (every
    \texttt{quant\_int8} kernel writes the per-tensor scale to host
    memory before the next \texttt{cublasGemmEx} can be queued).
  \item 800 kernel launches per token at $\sim$5--10 $\mu$s each: several
    ms of pure launch overhead per token.
  \item Naive attention kernel (1 block per head, scalar inner loops, no
    tensor core utilization).
  \item No kernel fusion: \texttt{rmsnorm} + \texttt{quant\_int8} +
    \texttt{matmul} are three separate launches where production engines
    fuse them.
\end{itemize}

\texttt{llama.cpp} represents years of optimization specifically in this
direction. A production-quality ternary inference engine adopting the
same techniques (kernel fusion, persistent attention, optimized {KV}
cache layout, attention with \texttt{mma} intrinsics) could plausibly
close the gap to parity or $1.5$--$2\times$. That work is outside the
scope of this paper.

The matmul-fabric-only result ($1.90\times$ faster than {INT8} {TC}) is
the defensible architectural claim; we draw a clear line between the
fabric (architectural property of the substrate) and the surrounding
plumbing (engineering work in any inference engine).
...
silicon to argue. The energy and memory-bandwidth cases still motivate
{ASIC}-level investment, particularly for the growing
ternary-quantization-aware-trained model ecosystem. We end with a clear
roadmap to close the remaining gaps: an {FPGA} prototype for J/op
measurement and a production-quality {CUDA} inference engine to
translate the matmul-fabric win into full-pipeline wall-clock parity.
...
\begin{itemize}
  ...
  \item End-to-end {CUDA} forward (packed weights): \texttt{cuda/ter\_cuda\_forward\_packed.cu}
\end{itemize}
